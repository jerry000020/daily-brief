...
def generate_ai_data() -> List[Dict]:
    """生成AI视频行业动态数据"""
...
def generate_stage_data() -> List[Dict]:
    """生成舞台设备资讯数据"""
...
def build_html(videos: List[Dict], ai_news: List[Dict], stage_products: List[Dict]) -> str:
    """构建符合要求的HTML页面"""
    ...
    <div class="flex items-center justify-between flex-wrap">
      ...
      <div class="flex items-center gap-4 mt-4 sm:mt-0">
        <div class="text-right">
          ...
          <div class="font-semibold">{date_str} {weekday}</div>
        </div>
...
'''
    ...
    # 添加视频卡片
    for video in videos:
        platform_color = "bg-blue-600" if video["platform"] == "FB" else "bg-pink-600"
        html += f'''
        ...
        <div class="flex flex-col sm:flex-row">
          <div class="relative sm:w-48 h-32 sm:h-auto">
            ...
            <div class="absolute bottom-2 left-2 flex gap-2">
              <span class="text-xs font-semibold {platform_color} text-white px-2 py-1 rounded-full">{video["platform"]}</span>
...
'''
  ...
  <footer class="px-4 mt-12 text-center text-gray-400 text-sm">
    <div class="bg-white rounded-xl shadow-lg p-6">
      ...
      <p class="mt-2">更新时间：{update_time}</p>
    </div>
  </footer>
...
def main():
    """主函数"""
    print("=" * 60)
    print("🚀 海外创意·AI视频·舞台设备 每日简报系统")
    print("=" * 60)
    
    # 生成数据
    print("\n📊 生成数据中...")
    videos = generate_video_data()
    ai_news = generate_ai_data()
    stage_products = generate_stage_data()
    print(f"   - 视频：{len(videos)} 条")
    print(f"   - AI动态：{len(ai_news)} 条")
    print(f"   - 设备资讯：{len(stage_products)} 条")
    
    # 构建HTML
    print("\n🎨 生成HTML页面...")
    html = build_html(videos, ai_news, stage_products)
